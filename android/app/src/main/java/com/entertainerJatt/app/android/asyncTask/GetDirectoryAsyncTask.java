...
import com.entertainerJatt.app.android.request.GetDirectoryRequest;
...
public class GetDirectoryAsyncTask extends AsyncTask<Object, String, ResultItem> {

    UserInfo userInfo;
    IAsyncTask iAsyncTask;

    public GetDirectoryAsyncTask(UserInfo usersInfo, IAsyncTask iAsyncTask) {
    ...
    protected ResultItem doInBackground(Object[] params) {
        if (Util.haveNetworkConnection(iAsyncTask.getContext()))
            try {
                return new GetDirectoryRequest().getJSONFromUrl(userInfo);
