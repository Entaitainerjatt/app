...
import com.entertainerJatt.app.android.request.SearchRequest;
...
public class SearchAsynTask extends AsyncTask<Object, String, ResultItem> {

    UserInfo userInfo;
    IAsyncTask iAsyncTask;

    public SearchAsynTask(UserInfo usersInfo, IAsyncTask iAsyncTask) {
    ...
    protected ResultItem doInBackground(Object[] params) {
        if (Util.haveNetworkConnection(iAsyncTask.getContext()))
            try {
                return new SearchRequest().getJSONFromUrl(userInfo);
