...
import com.entertainerJatt.app.android.request.GetNodeRequest;
...
public class GetNodeAsyncTask extends AsyncTask<Object, String, ResultItem> {

    UserInfo userInfo;
    IAsyncTask iAsyncTask;

    public GetNodeAsyncTask(UserInfo usersInfo, IAsyncTask iAsyncTask) {
    ...
    protected ResultItem doInBackground(Object[] params) {
        if (Util.haveNetworkConnection(iAsyncTask.getContext()))
            try {
                return new GetNodeRequest().getJSONFromUrl(userInfo);
