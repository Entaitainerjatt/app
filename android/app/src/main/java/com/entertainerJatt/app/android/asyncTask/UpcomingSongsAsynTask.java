...
import com.entertainerJatt.app.android.request.UpcomingSongsRequest;
...
public class UpcomingSongsAsynTask extends AsyncTask<Object, String, ResultItem> {

    UserInfo userInfo;
    IAsyncTask iAsyncTask;

    public UpcomingSongsAsynTask(UserInfo usersInfo, IAsyncTask iAsyncTask) {
    ...
    protected ResultItem doInBackground(Object[] params) {
        if (Util.haveNetworkConnection(iAsyncTask.getContext()))
            try {
                return new UpcomingSongsRequest().getJSONFromUrl(userInfo);
