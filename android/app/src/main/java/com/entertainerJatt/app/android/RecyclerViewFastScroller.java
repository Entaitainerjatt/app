...
import com.entertainerJatt.app.android.adapters.AlphabetAdapter;
...
public class RecyclerViewFastScroller extends LinearLayout implements
    ...
    public void OnItemClicked(int alphabetPosition, int position) {
        ...
        takeRecyclerViewScrollToAlphabetPosition(alphabets.get(position).word);
    }
    ...
    private void takeRecyclerViewScrollToAlphabetPosition(String position) {
        if (recyclerView == null || recyclerView.getAdapter() == null) {
            return;
        }
        ArrayList<HashMap<String, String>> DirectoryList = DirectoryAdapter.getList();
        for (int i = 0; i < DirectoryList.size(); i++) {
            String data = DirectoryList.get(i).get("title");
