...
import com.entertainerJatt.app.android.request.LatestSongsRequest;
...
public class LatestSongsAsynTask extends AsyncTask<Object, String, ResultItem> {

    UserInfo userInfo;
    IAsyncTask iAsyncTask;

    public LatestSongsAsynTask(UserInfo usersInfo, IAsyncTask iAsyncTask) {
    ...
    protected ResultItem doInBackground(Object[] params) {
        if (Util.haveNetworkConnection(iAsyncTask.getContext()))
            try {
                return new LatestSongsRequest().getJSONFromUrl(userInfo);
